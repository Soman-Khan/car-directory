...
if( isset($_REQUEST['iv-submit-listing']) && isset($_REQUEST['payment_gateway']) && $_REQUEST['iv-submit-listing']=='register' && $_REQUEST['payment_gateway']=='paypal'){	
				...
				$package_id=sanitize_text_field($_POST['package_id']);
					...
					if($userdata['user_login']!='' and $userdata['user_email']!='' and $userdata['user_pass']!='' ){
						...
						if ( !$user_id and email_exists($userdata['user_email']) == false ) {							
							 ...
							 $user->set_role('basic');
							 $userId=$user_id;
							 $expire_interval = get_post_meta($package_id, 'iv_property_package_initial_expire_interval', true);						
							 $initial_expire_type = get_post_meta($package_id, 'iv_property_package_initial_expire_type', true);
							 ...
							 update_user_meta($user_id, 'iv_property_package_id',$package_id);
							 include( wp_iv_property_ABSPATH. 'inc/signup-mail.php');
						} else {
							$iv_redirect = get_option( '_iv_property_registration');
							if(trim($iv_redirect)!=''){
			...
			$iv_property_package_cost= get_post_meta( $package_id,'iv_property_package_cost',true);
			$iv_property_package_recurring= get_post_meta( $package_id,'iv_property_package_recurring',true);
			$package_cost=$iv_property_package_cost;
			// Cheek here Coupon ************
			$trial_enable= get_post_meta( $package_id,'iv_property_package_enable_trial_period',true);
			if( $iv_property_package_recurring=='on'  ){
				$package_cost=get_post_meta($package_id, 'iv_property_package_recurring_cost_initial', true);			
			}
			if($package_cost >0){
					$currencyCode = (isset($paypal_api_currency)) ? $paypal_api_currency : 'USD';
					$sandbox = ($paypal_mode == 'live') ? '' : 'sandbox.';
					$paymentName = $package_name;
					$paymentDescription = $iv_property_package_cost.' '.$currencyCode.' for '.$package_name .' at '. get_bloginfo();
					$page_name_registration=get_option('_iv_property_registration' ); 
					...
					if($iv_property_package_recurring=='on'){
							$period= get_post_meta( $package_id,'iv_property_package_recurring_cycle_type',true);
							...
							$params = $urlParams + $recurring;
									...
									// CouponStart ******************************										
											...
											$dis_amount=0;
											$package_amount=$iv_property_package_cost;
												$post_count = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->posts WHERE post_title = '%s' and  post_type='iv_property_coupon'",$coupon_code ));	
												if(sizeof($post_count)>0){
													$coupon_name = $post_count->post_title;							 
													 $current_date=$today = date("m/d/Y");							 
													 $start_date=get_post_meta($post_count->ID, 'iv_property_coupon_start_date', true);
													 $end_date=get_post_meta($post_count->ID, 'iv_property_coupon_end_date', true);
													 $coupon_used=get_post_meta($post_count->ID, 'iv_property_coupon_used', true);
													 $coupon_limit=get_post_meta($post_count->ID, 'iv_property_coupon_limit', true);
													 $dis_amount=get_post_meta($post_count->ID, 'iv_property_coupon_amount', true);							 
													 $package_ids =get_post_meta($post_count->ID, 'iv_property_coupon_pac_id', true);
													 ...
													 if($today_time >= $start_time && $today_time<=$expire_time && $coupon_used<=$coupon_limit && $pac_found == '1' ){	
														$coupon_type= get_post_meta($post_count->ID, 'iv_property_coupon_type', true);
														update_post_meta($post_count->ID, 'iv_property_coupon_used', $coupon_used+1);
					...
					// Coupon End *****************************
									$iv_property_package_cost = $iv_property_package_cost- $dis_amount;
									$orderParams = array(
									'PAYMENTREQUEST_0_AMT' => $iv_property_package_cost,
									'PAYMENTREQUEST_0_SHIPPINGAMT' => '0',
									'PAYMENTREQUEST_0_CURRENCYCODE' => $currencyCode,
									'PAYMENTREQUEST_0_ITEMAMT' => $iv_property_package_cost
									);
									$itemParams = array(
									'L_PAYMENTREQUEST_0_NAME0' => $paymentName,
									'L_PAYMENTREQUEST_0_DESC0' => $paymentDescription,
									'L_PAYMENTREQUEST_0_AMT0' =>  $iv_property_package_cost,
					...
					if(is_array($response) && $response['ACK'] == 'Success') {
								...
								$peried=get_post_meta( $package_id,'iv_property_package_recurring_cycle_type',true);
								$cycle_count=get_post_meta( $package_id,'iv_property_package_recurring_cycle_count',true);
									update_user_meta($userId, 'iv_paypal_recurring_profile_amt', get_post_meta( $package_id,'iv_property_package_recurring_cost_initial',true));
									update_user_meta($userId, 'iv_property_payment_gateway','paypal-express'); 
									update_user_meta($userId, 'iv_paypal_recurring_profile_init_amt',get_post_meta( $package_id,'iv_property_package_cost',true));
									...
									update_user_meta($userId, 'iv_property_discount',$dis_amount);
									update_user_meta($userId, 'iv_property_package_id',$package_id);							
				 ...
				 //set Role Forr Free Package*******
					if(isset($userId)){
						    $user = new WP_User( $userId );
							 $role_package= get_post_meta( $package_id,'iv_property_package_user_role',true); 
							 if($role_package==""){
								 $role_package='Basic';
							 }
							 ...
							 update_user_meta($userId, 'iv_property_package_id',$package_id);
				// success Page
					$iv_redirect = get_option( '_iv_property_thank_you_page');
						if(trim($iv_redirect)!=''){
	...
	if($tokenRow){	
		...
		if( is_array($checkoutDetails) && ($checkoutDetails['ACK'] == 'Success') ) {
			...
			if($iv_property_package_recurring=='on'){
				...
				$recurring_cycle_count=	get_post_meta($package_id,'iv_property_package_recurring_cycle_count',true);
				...
				if( $trial_enable=='yes'  ){
						$initAmt='0';
						$period = get_post_meta( $package_id,'iv_property_package_recurring_trial_type',true);						
						$recurring_cycle_count=	get_post_meta( $package_id,'iv_property_package_trial_period_interval',true);	
				}	
				...
				$arb_limit	= get_post_meta( $package_id,'iv_property_package_recurring_cycle_limit',true); 
				if($arb_limit!=''){
					$recurringParams['TOTALBILLINGCYCLES']=$arb_limit;
				}
				$trial_enable= get_post_meta( $package_id,'iv_property_package_enable_trial_period',true); 
				if( $trial_enable=='yes' ){					
					$trial_amount= get_post_meta( $package_id,'iv_property_package_trial_amount',true); 
					$trial_period_interval= get_post_meta( $package_id,'iv_property_package_trial_period_interval',true); 
					$trial_type = get_post_meta( $package_id,'iv_property_package_recurring_trial_type',true);
				...
				if( is_array($recurringPayment) && $recurringPayment['ACK'] == 'Success' ) {
					...
					update_user_meta($userId, 'iv_property_exprie_date',$begins); 
					update_user_meta($userId, 'iv_property_package_id',$package_id);
					// Check here for success status*****	
						if($recurringPayment['PROFILESTATUS']=='PendingProfile'){
							update_user_meta($userId, 'iv_property_payment_status', 'pending');
						}else{
							$role_package= get_post_meta( $package_id,'iv_property_package_user_role',true); 
							update_user_meta($userId, 'iv_property_payment_status', 'success');
							$user = new WP_User( $userId );
							$user->set_role($role_package);						
						}
					//Start aktar mail module  
							$dis_amount=$discount;	
							 include( wp_iv_property_ABSPATH.'inc/order-mail.php');
				...
				if( is_array($singlePayment) && $singlePayment['ACK'] == 'Success') {
					...
					//New code for expire date
							 ...
							 update_user_meta($userId, 'iv_property_exprie_date',$exp_d); 
							 update_user_meta($userId, 'iv_property_package_id',$package_id);
					// End expire date
			...
			if($package_cost >0){
					...
					if(is_array($response) && $response['ACK'] == 'Success') {
								...
								$cycle_count=get_post_meta( $package_id,'iv_property_package_recurring_cycle_count',true);
									...
									update_user_meta($userId, 'iv_paypal_recurring_profile_period',$peried);	
									update_user_meta($userId, 'iv_paypal_recurring_cycle_count',$cycle_count);
									update_user_meta($userId, 'iv_property_discount','');
									update_user_meta($userId, 'iv_paypal_recurring_profile_desc',$recurringDescriptionFull);
					}	
...
if( isset($_REQUEST['iv-submit-balance']) &&  $_REQUEST['iv-submit-balance']=='addbalance' ){
	...
	if($amount >0){
			...
			if(is_array($response) && $response['ACK'] == 'Success') {
					...
					update_user_meta($userId, '_iv_property_balance_submit','');
			}					
			wp_redirect( 'https://www.'.$sandbox.'paypal.com/webscr?cmd=_express-checkout&token=' . urlencode($token) ); 
			exit; 
	 }
