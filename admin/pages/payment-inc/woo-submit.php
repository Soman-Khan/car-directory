<?php		
			...
			if(class_exists('WooCommerce' ) ) {  		
				...
				if($userdata['user_login']!='' and $userdata['user_email']!='' and $userdata['user_pass']!='' ){
					...
					if ( !$user_id and email_exists($userdata['user_email']) == false ) {							
						...
						$product_id= get_post_meta( $package_id,'iv_property_package_woocommerce_product',true); 
						...
						if($package_cost >0){  
							update_user_meta($user_id, 'iv_property_payment_woo','woo_new');
							global $woocommerce;
							...
							update_user_meta($user_id, 'iv_property_payment_status','success');							
	...
	if(isset($_REQUEST['iv-submit-upgrade']) &&  isset($_REQUEST['payment_gateway']) && $_REQUEST['iv-submit-upgrade']=='upgrade' && $_REQUEST['payment_gateway']=='woocommerce'){	 
		if(class_exists('WooCommerce' ) ) { 
			...
			update_user_meta($user_id, 'iv_property_package_id',$package_id);
			update_user_meta($user_id, 'iv_property_payment_woo','woo_update');
			// Add to cart****************							
			//login
