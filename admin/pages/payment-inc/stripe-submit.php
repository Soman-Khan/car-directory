<?php
	if( isset($_POST['iv-submit-stripe']) && $_POST['payment_gateway']=='stripe' && $_POST['iv-submit-stripe']=='register' ){	
		...
		$stripe_currency =get_post_meta($newpost_id, 'iv_property_stripe_api_currency',true);	
		...
		if(trim(get_post_meta( $package_id,'eplugins_stripe_plan',true))!=''){
			$package_post_name= trim(get_post_meta( $package_id,'eplugins_stripe_plan',true));
		}
		...
		if($package_cost >0){
			...
			if($iv_property_package_recurring!='on'){
				...
				$post_count = $wpdb->get_row("SELECT * FROM $wpdb->posts WHERE post_title = '" . $coupon_code . "' and  post_type='iv_property_coupon'");	
			...
			}	// for if($iv_property_package_recurring!='on'){
			// Coupon End *****************************
			...
			if($iv_property_package_recurring=='on'){
				...
				if( $trial_enable=='yes' ){
					...
					$package_cost_int=get_post_meta($package_id, 'iv_property_package_recurring_cost_initial', true);								
				...
				update_user_meta($userId, 'iv_property_payment_gateway','stripe'); 								
				if(isset($customer->id)){
					update_user_meta($userId, 'iv_property_stripe_cust_id', $customer->id);
				}							
				if(isset($customer->subscriptions->data[0]->id)){
					update_user_meta($userId, 'iv_property_stripe_subscrip_id', $customer->subscriptions->data[0]->id);
				}
				...
				if($payment_status=='active' or $payment_status=='trialing' ){
					...
					update_user_meta($userId, 'iv_property_exprie_date',$exprie); 
					update_user_meta($userId, 'iv_property_payment_status', 'success');
					}else{							 							
					update_user_meta($userId, 'iv_property_payment_status', 'pendding'); 
				}	 								  
