...
function iv_update_mailchamp_settings(){
	var search_params={
		"action"  : 	"iv_property_update_mailchamp_setting",	
...
function update_user_setting() {
				// New Block For Ajax*****
			var search_params={
				"action"  : 	"iv_property_update_user_settings",	
			...
			jQuery.ajax({					
				...
				data : search_params,
				success : function(response){
					var url = admindata.wp_iv_property_ADMINPATH+"admin.php?page=wp-iv_user-directory-admin&message=success";    						
...
function update_stripe_setting() {
				...
				var ajaxurl = admindata.ajaxurl;
				var search_params={
					"action"  : 	"iv_property_update_stripe_settings",	
				...
				jQuery.ajax({					
					...
					data : search_params,
					success : function(response){
							jQuery('#iv-loading').html('<div class="col-md-12 alert alert-success">Update Successfully. <a class="btn btn-success btn-xs" href="?page=wp-iv_property-payment-settings"> Go Payment Setting Page</aa></div>');
...
function iv_update_payment_settings_terms() {
				// New Block For Ajax*****
				var search_params={
					"action"  : 	"iv_property_update_payment_setting",	
...
function iv_update_page_settings(){
				var search_params={
					"action"  : 	"iv_property_update_page_setting",	
...
function iv_update_email_settings(){
				var search_params={
					"action"  : 	"iv_property_update_email_setting",	
...
function iv_update_account_settings(){
				var search_params={
					"action"  : 	"iv_property_update_account_setting",	
...
var search_params={
		"action"  : 	"iv_property_update_protected_setting",	
...
function update_profile_fields(){
		var ajaxurl = admindata.ajaxurl;
		var search_params = {
			"action": 		"iv_property_update_profile_fields",
...
function update_paypal_setting() {
				...
				// New Block For Ajax*****
				var search_params={
					"action"  : 	"iv_property_update_paypal_settings",	
				...
				jQuery.ajax({					
					...
					data : search_params,
					success : function(response){
						jQuery('#iv-loading').html('<div class="col-md-5 alert alert-success">Update Successfully. <a class="btn btn-success btn-xs" href="?page=wp-iv_property-payment-settings"> Go Payment Setting Page</aa></div>');
...
function  iv_update_payment_gateways_settings (){
		...
		var search_params = {
			"action": "iv_property_gateway_settings_update",
...
function update_the_package() {
	...
	jQuery("#loading").html(loader_image);
			...
			var search_params={
				"action"  : 	"iv_property_update_package",	
			...
			jQuery.ajax({					
				...
				data : search_params,
				success : function(response){						
					var url = admindata.wp_iv_property_ADMINPATH+"admin.php?page=wp-iv_property-package-all&form_submit=success";    						
					jQuery(location).attr('href',url);
				}
			});
...
function save_the_package() {
		...
		jQuery("#loading").html(loader_image);
				...
				// New Block For Ajax*****
				var search_params={
					"action"  : 	"iv_property_save_package",	
					"form_data":	jQuery("#package_form_iv").serialize(),
					"_wpnonce": admindata.packagenonce,	
				};
				jQuery.ajax({					
...
function iv_package_status_change(status_id,curr_status){
	...
	var ajaxurl = admindata.ajaxurl;
	var search_params = {
		"action": 	"iv_property_update_package_status",
		"status_id": status_id,	
	...
	image_gallery_frame.on( 'select', function() {
		...
		selection.map( function( attachment ) {
			...
			if ( attachment.id ) {							
				var ajaxurl = admindata.ajaxurl;
				var search_params = {
					"action": 	"iv_property_update_map_marker",
	...
	image_gallery_frame.on( 'select', function() {
		...
		selection.map( function( attachment ) {
			...
			if ( attachment.id ) {							
				var ajaxurl =admindata.ajaxurl;
				var search_params = {
					"action": 	"iv_property_update_cate_image",
					"attachment_id": attachment.id,
					"category_id": cat_image_id,
					"_wpnonce": admindata.catimage,
				};
...
function update_dir_fields(){
	var search_params = {
		"action": 		"iv_property_update_dir_fields",
...
function iv_update_coupon() {	
	// New Block For Ajax*****
	var search_params={
		"action"  : 	"iv_property_update_coupon",	
	...
	jQuery.ajax({					
		...
		data : search_params,
		success : function(response){
			var url = admindata.wp_iv_property_ADMINPATH+"admin.php?page=wp-iv_property-coupons-form&form_submit=success";
			jQuery(location).attr('href',url);
		}
	});
}
...
function iv_create_coupon() {	
	// New Block For Ajax*****
	var search_params={
		"action"  : 	"iv_property_create_coupon",	
		"form_data":	jQuery("#coupon_form_iv").serialize(), 
		"form_pac_ids": jQuery("#package_id").val(),
		"_wpnonce": admindata.coupon,
	};
