...
global $wpdb;
	...
	$userId=$current_user->ID;
		$payment_gateway=get_option('iv_property_payment_gateway');
		if($payment_gateway=='paypal-express'){
				...
				$payment_status=get_user_meta($userId, 'iv_property_payment_status', true);
				if($payment_status=='pending'){
					...
					$exprie_date= strtotime (get_user_meta($userId, 'iv_property_exprie_date', true));	
				...
				if($exprie_date < $current_date){
							...
							if(isset($recurringCheck['STATUS'])){
								if($recurringCheck['STATUS']=='Active'){
									...
									// Change exprie_date 
									$package_id=get_user_meta($userId,'iv_property_package_id',true); 
										$iv_property_exprie_date_old =get_user_meta($userId,'iv_property_exprie_date',true);
										...
										if($recurring_cycle_count=="" or $recurring_cycle_count==0){$recurring_cycle_count=1;}
										 $recurring_cycle_type= get_post_meta($package_id,'iv_property_package_recurring_cycle_type',true);
										$periodNum='';
									...
									update_user_meta($userId, 'iv_property_exprie_date', $new_exp_date); 
									// End exprie_date 
									//Add  History for Payment
										...
										if(get_post_meta($package_id,'iv_property_package_recurring',true)=='on'){
											$iv_property_package_cost =get_post_meta($package_id,'iv_property_package_recurring_cost_initial',true);
										}else{
										...
										$total_g = $iv_property_package_cost.' '.$api_currency;												
		...
		if($payment_gateway=='stripe'){
				...
				$payment_status=get_user_meta($userId, 'iv_property_payment_status', true);	
				if($payment_status=='pending'){								
						$cust_id = get_user_meta($userId,'iv_property_stripe_cust_id',true);
						$sub_id = get_user_meta($userId,'iv_property_stripe_subscrip_id',true);
						if($sub_id!=''){							
							try{
	...
	if($payment_gateway=='woocommerce'){ 
		...
		$status_result=0;
		$wpayment_status=get_user_meta($userId, 'iv_property_payment_woo', true);
		$package_id=get_user_meta($userId, 'iv_property_package_id', true);
		if($wpayment_status=='woo_new'){
			 include(wp_iv_property_DIR . '/admin/pages/payment-inc/woo_new_order.php');
		}				
		if($wpayment_status=='woo_update'){ 
			 include(wp_iv_property_DIR . '/admin/pages/payment-inc/woo_update_package.php');			
		}
		if($wpayment_status=='success'){ 
				$exprie_date= strtotime (get_user_meta($userId, 'iv_property_exprie_date', true));
				$current_date=strtotime(date('Y-m-d'));
			if(  $current_date > $exprie_date){ 
				include(wp_iv_property_DIR . '/admin/pages/payment-inc/woo_daily_check.php');
			}
		}
	}
}
