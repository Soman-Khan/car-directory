<?php
global $wpdb;			
	$email_body_cilent = get_option( 'iv_property_order_client_email');
	$order_email_subject_client = get_option( 'iv_property_order_client_email_sub');			
					
		$admin_mail = get_option('admin_email');	
		if( get_option( 'admin_email_iv_property' )==FALSE ) {
			$admin_mail = get_option('admin_email');						 
		}else{
			$admin_mail = get_option('admin_email_iv_property');								
		}						
	...
	$dis_amount =get_user_meta($userId, 'iv_property_discount',true);
	
	$package_amount_2=$iv_property_package_cost.' '.$api_currency;
	$discount_2= $dis_amount.' '.$api_currency;
	$total_g =($iv_property_package_cost - $dis_amount).' '.$api_currency;
	...
	// For Admin Email*****	
	$email_body_admin = get_option( 'iv_property_order_admin_email');
	$order_email_subject_admin = get_option( 'iv_property_order_admin_email_sub');			
