...
if($directory_url==""){$directory_url='property';}
	...
	if(sizeof($membership_users)>0){
		...
		{	
			$user_id= $row->ID;
			$reminder_day = get_option( 'iv_property_reminder_day');
			...
			if( $reminder_day >= $days ){
					...
					if(strtotime($exprie_send_email_date) != strtotime($exp_date) || $exprie_send_email_date=='' ){
						// Start Email Action
							...
							$email_body = get_option( 'iv_property_reminder_email');
							$signup_email_subject = get_option( 'iv_property_reminder_email_subject');			
	...
	if(sizeof($total_post)>0){
		...
		foreach ( $all_post as $row )								
		{
			...
			$post_author_id=$row->post_author;	
				...
				$exp_date= get_user_meta($post_author_id, 'iv_property_exprie_date', true);
				if($exp_date!=''){
					$package_id=get_user_meta($post_author_id,'iv_property_package_id',true);
					...
					$have_package_feature= get_post_meta($package_id,'iv_property_package_feature',true);											
